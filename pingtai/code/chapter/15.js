...
var ll;
var time;
  function runGame(plans, Display,user) {
	  var myDate = new Date();
	  nam=user;
      ...
      runLevel(new Level(plans[n]), Display, function(status) {
        if (status == "lost") {
		 ...
		 if(j>=localStorage.length){
			...
			ll=shuju.length;
			shuju[ll]={id: ll,name: nam, price:counte,time: time};		
			localStorage.setItem("paihang",JSON.stringify(shuju));
		 }
		 counte=0;
		 chayong();
...
var temp;
	...
	runGame(GAME_LEVELS, CanvasDisplay,decodeURIComponent(location.search.substr(4)));
